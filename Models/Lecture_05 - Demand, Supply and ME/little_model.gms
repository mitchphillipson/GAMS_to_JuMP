...
* This is not the C from slide 29 of the lecture notes. What is this C?
* Why is PY not present in this value?
$macro    C    ((theta*PL**(1-sigma)+(1-theta)*PK**(1-sigma))**(1/(1-sigma)))

* These are from slide 23 of the lecture notes. But why is AY 1?
* Also, where is y/ \bar{y} in AL and AK?
$macro    AY    (1)
$macro    AL    (-ld0*(C/PL)**sigma)
$macro    AK    (-kd0*(C/PK)**sigma)

* Where do these D's come from? And why is DK 0?
$macro    DY    (alpha*(PL*le0+PK*ke0)/PY)
$macro    DL    ((1-alpha)*(PL*le0+PK*ke0)/PL)
$macro    DK    (0)

* BY is 0 because an "output endowment" doesn't make sense (is this correct?)
$macro    BY    (0)
$macro    BL    (le0)
$macro    BK    (ke0)
